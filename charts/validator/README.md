...
![Version: 0.4.0-pre.3](https://img.shields.io/badge/Version-0.4.0--pre.3-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 0.58](https://img.shields.io/badge/AppVersion-0.58-informational?style=flat-square)
...
| ghost2.affinity | object | `{}` | pod Affinity spec applied validator |
| ghost2.argsOverride | list | `[]` | args override for the validator |
| ghost2.chainId | int | `1` | chain id for the "target" or "main" chain we use for the validator. Can be mainnet ethereum `1` or sepolia ethereum `11155111` |
| ghost2.chainName | string | `"eth"` | chain name for the "target" or "main" chain we use for the validator |
| ghost2.chainRpcUrl | string | `nil` | RPC url for the "target" or "main" chain we use for the validator. Can be mainnet ethereum or sepolia ethereum |
| ghost2.chainTxType | string | `"eip1559"` | chain tx type for the "target" or "main" chain we use for the validator. Can be mainnet ethereum `eip1559` or `legacy` |
| ghost2.commandOverride | list | `[]` | command override for the validator |
| ghost2.env | object | `{"normal":{}}` | Environment variable listing |
| ghost2.env.normal | object | `{}` | un-encrypted env vars passed to the pod |
| ghost2.ethConfig | object | `{}` | Provide ETH keys from existing secrets : **NB** use only existing secret OR env vars, do not provide both |
| ghost2.fullnameOverride | string | `""` | Override the release name to so tor-proxy can work with the default config. NB only change this if you know what you are doing |
| ghost2.image.pullPolicy | string | `"Always"` |  |
| ghost2.image.repository | string | `"ghcr.io/chronicleprotocol/ghost"` |  |
| ghost2.image.tag | string | `"0.58.1@sha256:af00d26b85c603960b8a11c3b6500f0993eef3d86d1afea7114fe272cb577474"` | Overrides the image tag whose default is the chart appVersion. |
| ghost2.imagePullSecrets | list | `[]` |  |
| ghost2.ingress | object | `{"annotations":{},"className":"","enabled":false,"hosts":[{"host":"chart-example.local","paths":[{"path":"/","pathType":"ImplementationSpecific","port":8000}]}],"tls":[]}` | Ingress for the validator (Do not enable ingress, as libp2p does not support path based routing yet) |
| ghost2.ingress.enabled | bool | `false` | Disabled by default, since there WAS a bug in libp2p port assignment for dns based routing |
| ghost2.liveness | object | `{"enabled":true,"livenessProbe":{"httpGet":{"path":"/healthz","port":9100},"initialDelaySeconds":30,"periodSeconds":60}}` | Liveness probe : restart the validator if the healthcheck endpoint is not reachable |
| ghost2.logFormat | string | `"text"` | Log format for the validator, can be one of `json`, `text` |
| ghost2.logLevel | string | `"info"` | Log level for the validator, can be one of `debug`, `info`, `warning`, `error` |
| ghost2.metrics.enabled | bool | `true` |  |
| ghost2.metrics.port | int | `9090` |  |
| ghost2.nameOverride | string | `""` |  |
| ghost2.nodeSelector | object | `{}` | Node selector for the validator |
| ghost2.podAnnotations | object | `{}` | Pod annotations for the validator |
| ghost2.podSecurityContext | object | `{}` | Pod security context for the validator |
| ghost2.readiness | object | `{"enabled":true,"readinessProbe":{"httpGet":{"path":"/healthz","port":9100},"initialDelaySeconds":30,"periodSeconds":60}}` | Readiness probe : stop the validator if the metrics endpoint is not reachable |
| ghost2.replicaCount | int | `1` |  |
| ghost2.resources | object | `{}` | Resources constraints for the validator, CPU, Memory, etc. |
| ghost2.rpcUrl | string | `nil` |  |
| ghost2.securityContext | object | `{}` | Security context for the validator |
| ghost2.service | object | `{"annotations":{},"ports":{"libp2p":{"port":8000,"protocol":"TCP"},"webapi":{"port":8080,"protocol":"TCP"}},"type":"LoadBalancer"}` | Service type for the validator |
| ghost2.service.annotations | object | `{}` | Annotations to add to the service |
| ghost2.service.ports.libp2p | object | `{"port":8000,"protocol":"TCP"}` | libp2p port for the validator service |
| ghost2.service.ports.webapi | object | `{"port":8080,"protocol":"TCP"}` | webapi listen port for the validator service |
| ghost2.service.type | string | `"LoadBalancer"` | Type of service for the validator, can also be `LoadBalancer`, `NodePort` is experimental |
| ghost2.serviceAccount.annotations | object | `{}` | Annotations to add to the service account |
| ghost2.serviceAccount.create | bool | `true` | Specifies whether a service account should be created |
| ghost2.serviceAccount.name | string | `""` | If not set and create is true, a name is generated using the fullname template |
| ghost2.tolerations | list | `[]` | Tolerations applied validator |
| ghost2.watchdogConfigReg | string | `"0x94Fea534aef6df5cF66C2DAE5CE0A05d10C068F3"` | WATCHDOG onchain config address |
| ghost2.webApi | object | `{"enabled":true,"listenAddr":"0.0.0.0:8080"}` | WEB API (tor-proxy) |
| ghost2.webApi.enabled | bool | `true` | Enables the web api and deploys the tor-proxy subchart |
| ghost2.webApi.listenAddr | string | `"0.0.0.0:8080"` | Listen address for the web api |
